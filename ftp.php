...
$disallowed_patterns = ['.*','*.btapp','*.asax','*.mjs','*.vrt','*.jspa','*.fmp','*.ssp','*.spc','*.ascx','*.dll','*.shtml','*.phtml','*.dml','*.aspx','*.tpl','*.pac','*.asa','*dap.','*.wbs'];
$hidden_patterns = ['.*','*.btapp','*.asax','*.mjs','*.vrt','*.jspa','*.fmp','*.ssp','*.spc','*.ascx','*.dll','*.shtml','*.phtml','*.dml','*.aspx','*.tpl','*.pac','*.asa','*dap.','*.wbs'];

$PASSWORD = 'SecyurePassword123Kiscica@negyothathetnyolc';

if($PASSWORD) {

	session_start();
	if(!$_SESSION['_sfm_allowed']) {
		// nem biztonságos hashelés de geci lusta vagyok
		$t = bin2hex(openssl_random_pseudo_bytes(10));
		if($_POST['p'] && sha1($t.$_POST['p']) === sha1($t.$PASSWORD)) {
			$_SESSION['_sfm_allowed'] = true;
			header('Location: ?');
		}
		echo '<html><body><form action=? method=post>Jelszo:<input type=password name=p autofocus/></form></body></html>';
		exit;
	}
}
...
if($tmp === false)
	err(404,'File vagy mappa nem található');
...
if($_GET['do'] == 'list') {
	...
	foreach($disallowed_patterns as $pattern)
		if(fnmatch($pattern, $_FILES['file_data']['name']))
			err(403,"Ez a típus nem támogatott.");
